...
=======
This pipeline processes transaction data, transforms it, and loads it into bronze, silver, and gold tables. It runs to ensure data is up-to-date for reporting and analysis. If it stops, downstream reports and analytics will be outdated.
...
9. Load computed metrics into `gold_merchant_performance` and `gold_daily_summary` tables using `load_gold(con, merchant_perf, daily_summary)`.
...
4. **Bronze Load Failure**
   - Validate `TRANSACTIONS_CLEAN` and `TRANSACTIONS_DIRTY` for malformed data.
   - Correct the data and rerun `load_bronze(con, transactions)`.
5. **Silver Transformation Failure**
   - Ensure all transactions have valid `merchant_id`.
   - Fix missing IDs and rerun `transform_bronze_to_silver(transactions, merchants)`.
>>>>>>> b8ad18c (day8 Done)
...
- Ensure `silver_transactions` has the correct number of transformed records.
- Check `gold_merchant_performance` and `gold_daily_summary` for expected metrics.
>>>>>>> b8ad18c (day8 Done)
